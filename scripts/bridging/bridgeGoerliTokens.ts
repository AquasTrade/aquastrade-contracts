...
import l1Artifacts from "../../ima_bridge/l1_artifacts.goerli.json";
import { SignerWithAddress } from "@nomiclabs/hardhat-ethers/dist/src/signers";

require("dotenv").config();

import { address as RubyMainnet } from "../../deployments/goerli/RubyTokenMainnet.json";
import l1ERC20Details from "../../deployment_addresses/l1_goerli_erc20s.json";

import ERC20Abi from "../../abi/@openzeppelin/contracts/token/ERC20/ERC20.sol/ERC20.json";

const SCHAIN_NAME = "staging-legal-crazy-castor";
...
const bridgeL1RubyToL2 = async (signer: SignerWithAddress) => {
  ...
  const amountRuby = ethers.utils.parseUnits("199998900", 18);
...
const main = async () => {
  ...
  // DONE await bridgeL1TokensToL2(signer, 'USDP', 3);
  // DONE await bridgeL1TokensToL2(signer, 'USDT', 3);
  // DONE await bridgeL1TokensToL2(signer, 'USDC', 3);
  // DONE await bridgeL1TokensToL2(signer, 'DAI', 3);
  // DONE await bridgeL1TokensToL2(signer, 'SKL', 3);
  // DONE await bridgeL1TokensToL2(signer, 'WBTC', 3);

  // DONE await bridgeEth(signer);
