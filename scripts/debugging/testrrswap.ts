...
const swapERC20ToUSDP = async (dryRun: boolean, erc20Addr: string, amountStr: string, signer: SignerWithAddress) => {
  ...
  console.log(
    `Current ${amountStr}${symbol} -> USDP (Starting ${symbol} balance: ${ethers.utils.formatUnits(
      balance,
      decimals,
    )})`,
  );
  ...
  console.log(
    `Should recieve ${ethers.utils.formatUnits(
      tokenOutAmount,
      decimals,
    )}${symbol} (Current ${symbol} balance: ${ethers.utils.formatUnits(balance, decimals)})`,
  );
...
const swapERC20ToUSDT = async (dryRun: boolean, erc20Addr: string, amountStr: string, signer: SignerWithAddress) => {
  ...
  console.log(
    `Current ${amountStr}${symbol} -> USDT (Starting ${symbol} balance: ${ethers.utils.formatUnits(
      balance,
      decimals,
    )})`,
  );
  ...
  const feeMultiplier = await nftAdmin.calculateAmmSwapFeeDeduction(account.address);
  const amountsOut = await ammRouter.getAmountsOut(
    tokenInAmount,
    [tokenIn.address, stableTokenIn.address],
    feeMultiplier,
  );
...
const main = async () => {
  ...
  await swapERC20ToUSDT(
    true, // true to simulate and not swap
    ETHC_ADDR,
    "0.001",
    signer,
  );
