...
const swapExactTokensForTokens = async (router: UniswapV2Router02, tokenA: string, tokenB: string, amountIn: BigNumber, amountOutMin: BigNumber, to: string, deadline: BigNumber ) => {
...
const main = async () => {
  ...
  await swapExactTokensForTokens(router, usdcAddr, usdtAddr, stableAmountIn, stableAmountOutMin, deployer.address, deadline)
