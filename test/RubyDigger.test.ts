...
describe("RubyDigger", function () {
  ...
  beforeEach(async function () {
    ...
    await deploy(this, [
      ["rubyDigger", this.RubyDigger, [this.factory.address, this.mine.address, this.ruby.address, this.weth.address]],
    ]);
  ...
  describe("setBridge", function () {
    it("does not allow to set bridge for Ruby", async function () {
      await expect(this.rubyDigger.setBridge(this.ruby.address, this.weth.address)).to.be.revertedWith(
        "RubyDigger: Invalid bridge",
      );
    });

    it("does not allow to set bridge for WETH", async function () {
      await expect(this.rubyDigger.setBridge(this.weth.address, this.ruby.address)).to.be.revertedWith(
        "RubyDigger: Invalid bridge",
      );
    });

    it("does not allow to set bridge to itself", async function () {
      await expect(this.rubyDigger.setBridge(this.dai.address, this.dai.address)).to.be.revertedWith(
        "RubyDigger: Invalid bridge",
      );
    });
  ...
  describe("convert", function () {
    ...
    it("reverts if caller is not EOA", async function () {
      ...
      await expect(this.exploiter.convert(this.ruby.address, this.weth.address)).to.be.revertedWith(
        "RubyDigger: must use EOA",
      );
    });

    it("reverts if pair does not exist", async function () {
      await expect(this.rubyDigger.convert(this.mic.address, this.micUSDC.address)).to.be.revertedWith(
        "RubyDigger: Invalid pair",
      );
    });

    it("reverts if no path is available", async function () {
      await this.micUSDC.transfer(this.rubyDigger.address, getBigNumber(1));
      await expect(this.rubyDigger.convert(this.mic.address, this.usdc.address)).to.be.revertedWith(
        "RubyDigger: Cannot convert",
      );
      expect(await this.ruby.balanceOf(this.rubyDigger.address)).to.equal(0);
  ...
  describe("convertMultiple", function () {
    it("should allow to convert multiple", async function () {
      await this.daiEth.transfer(this.rubyDigger.address, getBigNumber(1));
      await this.rubyEth.transfer(this.rubyDigger.address, getBigNumber(1));
      await this.rubyDigger.convertMultiple(
