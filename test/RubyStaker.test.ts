...
describe("RubyStaker", function () {
  ...
  it("reward minter should be set correctly", async function () {
    ...
    expect(await this.staker.setRewardMinter(this.owner.address))
      .to.emit(this.staker, "RewardMinterSet")
  ...
  it("reward should be added correctly", async function () {
    expect(await this.staker.addReward(this.ruby.address, this.owner.address))
      .to.emit(this.staker, "RewardDataRegistered")
      .withArgs(this.ruby.address, this.owner.address);
  ...
  it("reward distributor should be approved correctly", async function () {
    ...
    expect(await this.staker.approveRewardDistributor(1, this.bob.address, true))
      .to.emit(this.staker, "RewardDistributorApproved")
      .withArgs(this.ruby.address, this.bob.address, true);
    ...
    expect(await this.staker.approveRewardDistributor(1, this.bob.address, false))
      .to.emit(this.staker, "RewardDistributorApproved")
      .withArgs(this.ruby.address, this.bob.address, false);
    rewardDistributor2Set = await this.staker.rewardDistributors(1, this.bob.address);
    expect(rewardDistributor2Set).to.be.eq(false);
  });
  ...
  it("single user staking should work as expected", async function () {
    ...
    expect(await this.staker.connect(this.bob).stake(stakeAmount, false))
      .to.emit(this.staker, "Staked")
      .withArgs(this.bob.address, stakeAmount);
  ...
  it("multiple user staking should work as expected", async function () {
    ...
    expect(await this.staker.connect(this.bob).stake(stakeAmounts[0], false))
      .to.emit(this.staker, "Staked")
      .withArgs(this.bob.address, stakeAmounts[0]);
    expect(await this.staker.connect(this.bob).stake(stakeAmounts[1], false))
      .to.emit(this.staker, "Staked")
      .withArgs(this.bob.address, stakeAmounts[1]);
    expect(await this.staker.connect(this.carol).stake(stakeAmounts[2], false))
      .to.emit(this.staker, "Staked")
      .withArgs(this.carol.address, stakeAmounts[2]);
  ...
  it("multiple user minting should work as expected", async function () {
    ...
    expect(await this.staker.mint(this.bob.address, mintAmounts[0]))
      .to.emit(this.staker, "Staked")
      .withArgs(this.bob.address, mintAmounts[0]);
    ...
    expect(await this.staker.mint(this.bob.address, mintAmounts[1]))
      .to.emit(this.staker, "Staked")
      .withArgs(this.bob.address, mintAmounts[1]);
    expect(await this.staker.mint(this.bob.address, mintAmounts[2]))
      .to.emit(this.staker, "Staked")
      .withArgs(this.bob.address, mintAmounts[2]);

    await advanceTimeByTimestamp(this.rewardDuration * 2);

    expect(await this.staker.mint(this.bob.address, mintAmounts[3]))
      .to.emit(this.staker, "Staked")
      .withArgs(this.bob.address, mintAmounts[3]);
  ...
  it("single user locking should work as expected", async function () {
    ...
    expect(await this.staker.connect(this.bob).stake(stakeAmount, true))
      .to.emit(this.staker, "Staked")
      .withArgs(this.bob.address, stakeAmount);
  ...
  it("multiple user locking should work as expected", async function () {
    ...
    expect(await this.staker.connect(this.bob).stake(stakeAmounts[0], true))
      .to.emit(this.staker, "Staked")
      .withArgs(this.bob.address, stakeAmounts[0]);
    expect(await this.staker.connect(this.bob).stake(stakeAmounts[1], true))
      .to.emit(this.staker, "Staked")
      .withArgs(this.bob.address, stakeAmounts[1]);
    const block = await ethers.provider.getBlock();
    const lockedUntil = Math.floor(block.timestamp / this.rewardDuration) * this.rewardDuration + this.lockDuration;
    ...
    await advanceTimeByTimestamp(this.rewardDuration);
    expect(await this.staker.connect(this.bob).stake(stakeAmounts[2], true))
      .to.emit(this.staker, "Staked")
      .withArgs(this.bob.address, stakeAmounts[2]);
    await advanceTimeByTimestamp(this.rewardDuration * 3);
    expect(await this.staker.connect(this.bob).stake(stakeAmounts[3], true))
      .to.emit(this.staker, "Staked")
      .withArgs(this.bob.address, stakeAmounts[3]);
  ...
  it("locks expiring should work as expected", async function () {
    ...
    expect(await this.staker.connect(this.bob).withdrawExpiredLocks())
      .to.emit(this.staker, "ExpiredLocksWithdrawal")
      .withArgs(this.bob.address, stakeAmounts[0] + stakeAmounts[1]);
  ...
  it("withdrwing unlocked stake should work as expected", async function () {
    ...
    expect(await this.staker.connect(this.bob).stake(stakeAmount, false))
      .to.emit(this.staker, "Staked")
      .withArgs(this.bob.address, stakeAmount);
    expect(await this.staker.connect(this.bob).withdraw(stakeAmount))
      .to.emit(this.staker, "Withdrawal")
      .withArgs(this.bob.address, stakeAmount);
  ...
  it("withdrawing partial unlocked stake should work as expected", async function () {
    ...
    expect(await this.staker.connect(this.bob).stake(stakeAmount, false))
      .to.emit(this.staker, "Staked")
      .withArgs(this.bob.address, stakeAmount);
    expect(await this.staker.connect(this.bob).withdraw(4000))
      .to.emit(this.staker, "Withdrawal")
      .withArgs(this.bob.address, 4000);
  ...
  it("withdrawing unlocked earned balance should work as expected", async function () {
    ...
    expect(await this.staker.connect(this.bob).stake(stakeAmount, false))
      .to.emit(this.staker, "Staked")
      .withArgs(this.bob.address, stakeAmount);
    expect(await this.staker.mint(this.bob.address, mintAmount))
      .to.emit(this.staker, "Staked")
      .withArgs(this.bob.address, mintAmount);
  ...
  it("withdrawing unlocked and earned balance with penalty should work as expected", async function () {
    ...
    await expect(this.staker.connect(this.bob).withdraw(25001)).to.be.revertedWith(
      "RubyStaker: Insufficient balance after penalty",
    );

    expect(await this.staker.connect(this.bob).withdraw(25000))
      .to.emit(this.staker, "Withdrawal")
      .withArgs(this.bob.address, 25000);
  ...
  it("withdrawing unlocked and earned partial balance should work as expected", async function () {
    ...
    expect(await this.staker.connect(this.bob).withdraw(15000))
      .to.emit(this.staker, "Withdrawal")
      .withArgs(this.bob.address, 15000);
  ...
  it("withdrawing earned balance without penalty should work as expected", async function () {
    ...
    await expect(this.staker.connect(this.bob).withdraw(totalMintAmount + 1)).to.be.revertedWith(
      "RubyStaker: Insufficient unlocked balance",
    );

    expect(await this.staker.connect(this.bob).withdraw(totalMintAmount))
      .to.emit(this.staker, "Withdrawal")
      .withArgs(this.bob.address, totalMintAmount);
  ...
  it("withdrawing earned partial balance with penalty should work as expected", async function () {
    ...
    await expect(this.staker.connect(this.bob).withdraw(60000 + 1)).to.be.revertedWith(
      "RubyStaker: Insufficient balance after penalty",
    );

    expect(await this.staker.connect(this.bob).withdraw(60000))
      .to.emit(this.staker, "Withdrawal")
      .withArgs(this.bob.address, 60000);
  ...
  it("getting penalties as locker should work as expected", async function () {
    ...
    expect(await this.staker.mint(this.carol.address, mintAmount))
      .to.emit(this.staker, "Staked")
      .withArgs(this.carol.address, mintAmount);
    await this.ruby.transfer(this.staker.address, mintAmount);
  ...
  it("only locked users should receive penalties", async function () {
    ...
    expect(await this.staker.connect(this.bob).stake(stakeAmounts[1], true))
      .to.emit(this.staker, "Staked")
      .withArgs(this.bob.address, stakeAmounts[1]);
    expect(await this.staker.connect(this.carol).stake(stakeAmounts[2], true))
      .to.emit(this.staker, "Staked")
      .withArgs(this.carol.address, stakeAmounts[2]);
  ...
  it("regular rewards should work as expected", async function () {
    ...
    expect(await this.staker.connect(this.bob).stake(stakeAmounts[0], true))
      .to.emit(this.staker, "Staked")
      .withArgs(this.bob.address, stakeAmounts[0]);
    expect(await this.staker.connect(this.carol).stake(stakeAmounts[1], false))
      .to.emit(this.staker, "Staked")
      .withArgs(this.carol.address, stakeAmounts[1]);
    ...
    const rewardAmounts = [ethers.utils.parseUnits("100000", 18), ethers.utils.parseUnits("500000", 18)];
