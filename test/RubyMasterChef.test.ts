...
describe("RubyMasterChef", function () {
  ...
  context("With ERC/LP token added to the field and using SimpleRewarderPerSec", function () {
    ...
    it("should only allow RubyMasterChef to call onRubyReward", async function () {
      ...
      await expect(this.rewarder.onRubyReward(this.bob.address, "100")).to.be.revertedWith("onlyRubyMasterChef: only RubyMasterChef can call this function") // t-10
    ...
    it("should allow rewarder to be set and removed mid farming", async function () {
      ...
      expect((await this.chef.pendingTokens(0, this.bob.address)).pendingRuby).to.be.within(81, 86)
      expect((await this.chef.pendingTokens(0, this.bob.address)).bonusTokenAddress).to.equal(this.partnerToken.address)
      expect((await this.chef.pendingTokens(0, this.bob.address)).bonusTokenSymbol).to.equal("SUSHI")
    ...
    it("should give out RUBYs only after farming time", async function () {
      ...
      expect(await this.ruby.balanceOf(this.bob.address)).to.be.within(90, 95)
      ...
      // At this point:
      //   Bob should have:
      //     - 90 + 5*9 = 135 (+5) RubyToken
      //     - 2520 + 5*40 = 2720 (+40) PartnerToken
      //   Treasury should have: 15*1 = 15 (+2)
      expect(await this.ruby.balanceOf(this.bob.address)).to.be.within(135, 140)
    ...
    it("should distribute RUBYs properly for each staker", async function () {
      ...
      //   MasterChef should have: 100 - 51 - 10 = 39 (+10)
      ...
      expect(await this.ruby.balanceOf(this.alice.address)).to.be.within(51 - this.tokenOffset, 56 + this.tokenOffset)
      ...
      expect(await this.ruby.balanceOf(this.treasury.address)).to.be.within(10 - this.tokenOffset, 12 + this.tokenOffset)
      expect(await this.ruby.balanceOf(this.chef.address)).to.be.within(39 - this.tokenOffset, 49 + this.tokenOffset)
      ...
      //   MasterChef should have: 39 + 100 - 55.7 - 10 = 73.3 (+10)
      ...
      expect(await this.ruby.balanceOf(this.bob.address)).to.be.within(55 - this.tokenOffset, 62 + this.tokenOffset)
      expect(await this.partnerToken.balanceOf(this.bob.address)).to.be.within(247 - this.tokenOffset, 287 + this.tokenOffset)

      expect(await this.ruby.balanceOf(this.carol.address)).to.equal("0")
      expect(await this.partnerToken.balanceOf(this.carol.address)).to.equal("0")

      expect(await this.ruby.balanceOf(this.treasury.address)).to.be.within(20 - this.tokenOffset, 22 + this.tokenOffset)
      expect(await this.ruby.balanceOf(this.chef.address)).to.be.within(73 - this.tokenOffset, 83 + this.tokenOffset)
      ...
      expect(await this.ruby.balanceOf(this.alice.address)).to.be.within(104 - this.tokenOffset, 109 + this.tokenOffset)
      expect(await this.partnerToken.balanceOf(this.alice.address)).to.be.within(463 - this.tokenOffset, 503 + this.tokenOffset)
      // Bob should have:
      //  - 55.7 + 10*9*15/65 + 10*9*15/45 = 102.46 (+5) RubyToken
      //  - 247 + 10*40*15/65 + 10*40*15/45 = 472 (+40) PartnerToken
      expect(await this.ruby.balanceOf(this.bob.address)).to.be.within(102 - this.tokenOffset, 107 + this.tokenOffset)
      ...
      expect(await this.ruby.balanceOf(this.carol.address)).to.be.within(239 - this.tokenOffset, 244 + this.tokenOffset)
      ...
      expect(await this.ruby.balanceOf(this.treasury.address)).to.be.within(50 - this.tokenOffset, 52 + this.tokenOffset)
      // MasterChefRuby should have nothing
      expect(await this.ruby.balanceOf(this.chef.address)).to.be.within(0, 0 + this.tokenOffset)
    ...
    it("should give proper RUBYs allocation to each pool", async function () {
      ...
      expect((await this.chef.pendingTokens(0, this.alice.address)).pendingRuby).to.be.within(90 - this.tokenOffset, 95 + this.tokenOffset)
      ...
      expect((await this.chef.pendingTokens(0, this.alice.address)).pendingRuby).to.be.within(105 - this.tokenOffset, 110 + this.tokenOffset)
      ...
      expect((await this.chef.pendingTokens(0, this.alice.address)).pendingRuby).to.be.within(120 - this.tokenOffset, 125 + this.tokenOffset)
      expect(await this.rewarder.pendingTokens(this.alice.address)).to.be.within(800, 840)

      expect((await this.chef.pendingTokens(1, this.bob.address)).pendingRuby).to.be.within(30 - this.tokenOffset, 35 + this.tokenOffset)
      ...
      expect(await this.ruby.balanceOf(this.alice.address)).to.be.within(123 - this.tokenOffset, 128 + this.tokenOffset)
      ...
      //   - 0 PartnerToken
      expect(await this.ruby.balanceOf(this.bob.address)).to.be.within(42 - this.tokenOffset, 47 + this.tokenOffset)
      expect(await this.rewarder.pendingTokens(this.bob.address)).to.equal(0)
    })
    ...
    it("should give proper RUBYs after updating emission rate", async function () {
      ...
      //   - 100*10*0.9 = 900 (+5) RubyToken
      //   - 100*40 = 4000 (+40) PartnerToken
      await advanceTimeAndBlock(100) // t+110
      expect((await this.chef.pendingTokens(0, this.alice.address)).pendingRuby).to.be.within(900,905)
      ...
      //   - 900 + 1*10*0.9 + 4*4*0.9 = 923.4 (+5) RubyToken
      //   - 4000 + 5*40 = 4200 (+40) PartnerToken
      await advanceTimeAndBlock(4) // t+115
      expect((await this.chef.pendingTokens(0, this.alice.address)).pendingRuby).to.be.within(923, 928)
      ...
      //   - 923.4 + 21*4*0.9 = 999 (+5) RubyToken
      //   - 4200 + 1*40 + 20*90 = 6040 (+90) PartnerToken
      await advanceTimeAndBlock(20) // t+136
      expect((await this.chef.pendingTokens(0, this.alice.address)).pendingRuby).to.be.within(999, 1004)
