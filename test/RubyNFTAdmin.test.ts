...
describe("NFTAdmin", function () {
  ...
  it("Minter should be added correctly", async function () {
    ...
    await expect(this.nftAdmin.mintProfileNFT(this.owner.address)).to.emit(this.rubyProfileNft, "Transfer").withArgs(ethers.constants.AddressZero, this.owner.address, 0);

    await expect(this.nftAdmin.setMinter(this.owner.address, false)).to.emit(this.nftAdmin, "MinterSet").withArgs(this.owner.address, false);
    expect(await this.nftAdmin.minters(this.owner.address)).to.be.eq(false);
  ...
  it("Profile NFT address should be set correctly", async function () {
    ...
    await expect(this.nftAdmin.connect(this.otherUser).setProfileNFT(newProfileNFTAddress)).to.be.revertedWith("Ownable: caller is not the owner");
    await expect(this.nftAdmin.setProfileNFT(newProfileNFTAddress)).to.emit(this.nftAdmin, "RubyProfileNFTset").withArgs(newProfileNFTAddress);
  ...
  it("Free Swap NFT address should be set correctly", async function () {
    ...
    await expect(this.nftAdmin.connect(this.otherUser).setFreeSwapNFT(newFreeSwapNFTAddress)).to.be.revertedWith("Ownable: caller is not the owner");
    await expect(this.nftAdmin.setFreeSwapNFT(newFreeSwapNFTAddress)).to.emit(this.nftAdmin, "FreeSwapNFTSet").withArgs(newFreeSwapNFTAddress);
  ...
  it("ProfileNFT should be minted correctly", async function () {
    ...
    await expect(this.nftAdmin.mintProfileNFT(this.otherUser.address)).to.emit(this.rubyProfileNft, "Transfer").withArgs(ethers.constants.AddressZero, this.otherUser.address, 0);
    ...
    await expect(this.rubyProfileNft.connect(this.otherUser).transferFrom(this.otherUser.address, this.owner.address, 0)).to.emit(this.rubyProfileNft, "Transfer").withArgs(this.otherUser.address, this.owner.address, 0);
    expect(await this.rubyProfileNft.balanceOf(this.otherUser.address)).to.be.eq(0);
    expect(await this.rubyProfileNft.balanceOf(this.owner.address)).to.be.eq(1);

    // an NFT should be minted to the same user again
    await expect(this.nftAdmin.mintProfileNFT(this.otherUser.address)).to.emit(this.rubyProfileNft, "Transfer").withArgs(ethers.constants.AddressZero, this.otherUser.address, 1);
    expect(await this.rubyProfileNft.balanceOf(this.otherUser.address)).to.be.eq(1);
  ...
  it("Swap fee deduction should be calculated correctly", async function () {
    ...
    await expect(this.rubyFreeSwapNft.mint(this.otherUser.address)).to.emit(this.rubyFreeSwapNft, "Transfer").withArgs(ethers.constants.AddressZero, this.otherUser.address, 0);
