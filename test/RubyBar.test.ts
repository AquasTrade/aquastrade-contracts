...
describe("RubyBar", function () {
  before(async function () {
    this.RubyToken = await ethers.getContractFactory("RubyToken");
    this.RubyBar = await ethers.getContractFactory("RubyBar");
  ...
  beforeEach(async function () {
    ...
    this.bar = await this.RubyBar.deploy(this.ruby.address);
  ...
  it("should not allow enter if not enough approve", async function () {
    ...
    await this.ruby.approve(this.bar.address, "50");
    await expect(this.bar.enter("100")).to.be.revertedWith("ERC20: transfer amount exceeds allowance");
    await this.ruby.approve(this.bar.address, "100");
  ...
  it("should work with more than one participant", async function () {
    await this.ruby.approve(this.bar.address, "100");
    await this.ruby.connect(this.bob).approve(this.bar.address, "100", { from: this.bob.address });
    ...
    expect(await this.ruby.balanceOf(this.bar.address)).to.equal("30");
    // RubyBar get 20 more RUBYs from an external source.
    await this.ruby.connect(this.carol).transfer(this.bar.address, "20", { from: this.carol.address });
    ...
    expect(await this.ruby.balanceOf(this.bar.address)).to.equal("52");
