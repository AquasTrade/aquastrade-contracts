...
describe("RubyNFT", function () {
  beforeEach(async function () {
    ...
    this.rubyNFT = await deployRubyNFT(
      this.owner.address,
      this.name,
      this.symbol,
      this.description,
  ...
  it("Minter should be added correctly and minting should be successful", async function () {
    ...
    await expect(this.rubyNFT.setMinter(this.owner.address, true))
      .to.emit(this.rubyNFT, "MinterSet")
    ...
    await expect(this.rubyNFT.mint(this.owner.address))
      .to.emit(this.rubyNFT, "Transfer")
    ...
    await expect(this.rubyNFT.setMinter(this.owner.address, false))
      .to.emit(this.rubyNFT, "MinterSet")
  ...
  it("Description should be set correctly", async function () {
    ...
    await expect(this.rubyNFT.connect(this.otherUser).setDescription(description)).to.be.revertedWith(
      "Ownable: caller is not the owner",
    );

    await expect(this.rubyNFT.setDescription(description))
      .to.emit(this.rubyNFT, "DescriptionSet")
  ...
  it("Visual appearance should be set correctly", async function () {
    ...
    await expect(this.rubyNFT.connect(this.otherUser).setVisualAppearance(visualAppearance)).to.be.revertedWith(
      "Ownable: caller is not the owner",
    );
    await expect(this.rubyNFT.setVisualAppearance(visualAppearance))
      .to.emit(this.rubyNFT, "VisualAppearanceSet")
      .withArgs(visualAppearance);
