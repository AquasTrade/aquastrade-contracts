...
import { deployAMM } from "../utilities/deployment"
describe("UniswapV2Factory", function () {
  ...
  it("Pair creator should not be set when the sender is not the admin", async function () {
    ...
    await  expect(this.factory.connect(this.user2).setPairCreator(this.user2.address)).to.be.revertedWith("UniswapV2: FORBIDDEN")
  });
  ...
  it("New admin should not be set when the sender is not the current admin", async function () {
    ...
    await expect(this.factory.connect(this.user2).setAdmin(this.user2.address)).to.be.revertedWith("UniswapV2: FORBIDDEN");
  });
