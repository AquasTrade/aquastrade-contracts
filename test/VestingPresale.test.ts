...
import { advanceTimeByTimestamp } from "./utilities";
...
describe("VestingPresale", function () {
  ...
  beforeEach(async function () {
    ...
    this.presale = await this.Vester.deploy(
  ...
  it("beneficiary 1 day 1 token", async function () {
    ...
    expect(await this.presale.connect(this.beneficiary).release(this.ruby.address, amt)).to.emit(
      this.presale,
      "TokensReleased",
    );
  ...
  it("beneficiary 1 day 1 token (request more)", async function () {
    ...
    expect(await this.presale.connect(this.beneficiary).release(this.ruby.address, 1000)).to.emit(
      this.presale,
      "TokensReleased",
    );
  ...
  it("beneficiary sequential", async function () {
    ...
    expect(await this.presale.connect(this.beneficiary).release(this.ruby.address, 10)).to.emit(
      this.presale,
      "TokensReleased",
    );
    expect(await this.ruby.balanceOf(this.presale.address)).to.be.within(3639, 3641);
    ...
    // none ready
    await expect(this.presale.connect(this.beneficiary).release(this.ruby.address, 10)).to.be.revertedWith(
  ...
  it("non-beneficiary cannot release", async function () {
    ...
    await expect(this.presale.connect(this.other).release(this.ruby.address, amt)).to.be.revertedWith(
  ...
  it("non-beneficiary cannot transfer beneficiary", async function () {
    await expect(this.presale.connect(this.other).transferBeneficiary(this.other.address)).to.be.revertedWith(
      "Vester: not beneficiary",
    );
  });
  ...
  it("beneficiary transfer", async function () {
    ...
    expect(await this.presale.connect(this.other).release(this.ruby.address, amt)).to.emit(
      this.presale,
      "TokensReleased",
    );
    expect(await this.ruby.balanceOf(this.presale.address)).to.be.within(3639, 3641);
  ...
  it("non-owner cannot revoke", async function () {
    ...
    await expect(this.presale.connect(this.other).revoke(this.ruby.address)).to.be.revertedWith(
