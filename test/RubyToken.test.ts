...
describe("RubyToken", function () {
  ...
  it("should only allow owner to mint token", async function () {
    ...
    await expect(this.ruby.connect(this.bob).mint(this.carol.address, "1000", { from: this.bob.address })).to.be.revertedWith(
  ...
  it("should fail if you try to do bad transfers", async function () {
    await this.ruby.mint(this.alice.address, "100")
    await expect(this.ruby.transfer(this.carol.address, "110")).to.be.revertedWith("ERC20: transfer amount exceeds balance")
    await expect(this.ruby.connect(this.bob).transfer(this.carol.address, "1", { from: this.bob.address })).to.be.revertedWith(
  ...
  it("should not exceed max supply of 200m", async function () {
    ...
    await expect(this.ruby.mint(this.alice.address, maxAmountWei.add(1))).to.be.revertedWith("RUBY::mint: Cannot exceed max supply.")
