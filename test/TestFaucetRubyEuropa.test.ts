...
describe("FaucetRubyEuropa", function () {
  ...
  it("User topup limited", async function () {
    ...
    expect(
      await ethers.provider.getBalance(this.faucet.address),
      "faucet has not dispensed more eth").to.be.eq(faucetBal);
  });

  it("Only owner can withdraw remaining", async function () {
    await expect(
      this.faucet.connect(this.burn).withdraw(),
      "user can't withdraw").to.be.revertedWith('Ownable: caller is not the owner');

    expect(
      await ethers.provider.getBalance(this.owner.address),
      "owner has starting funds").to.be.eq(this.ownerBefore);
    ...
    expect(
      await ethers.provider.getBalance(this.faucet.address),
      "faucet is empty").to.be.eq(0);
    expect(
      await ethers.provider.getBalance(this.owner.address),
      "owner has old faucet funds").to.be.gt(this.ownerBefore);
