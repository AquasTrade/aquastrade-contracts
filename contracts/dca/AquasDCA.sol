...
contract AquasDCA is Ownable {
    ...
    constructor(
        ...
        require(_usd != address(0), "Factory: Missing USD_ADDRESS address");
    ...
    // this assumes all pairs are with usdp base, need to recode 
    // maybe use the amm pair address pool.address instead of using the tokenAddresses 
    // as is , I can still deploy AQUA - USDC 
    function GetStorageAddressUsingToken(address _tokenXYZ) public returns (address) {
        require(_tokenXYZ != address(0), "AquasDCA: DCAStorageID: incorrect token address");
...
// Must give Factory Admin BURNER_ROLE for this functionality 
  ...
  // works with any erc20 
     function _transferBaseToStorage(
        address _tokenXYZ,
        uint256 _amount,
        address _storageContractAddress
    ) private {
    ...
    function _transferUSDPToStorage(uint256 _amount, address _storageContractAddress) private {
        ...
        require(token.balanceOf(usersWallet) >= _amount, "Insufficient token (USDC_ADDRESS) balance");
        token.safeTransferFrom(usersWallet, _storageContractAddress, _amount);
    }

    function _deployDCAStorage(address _tokenXYZ) private returns (address) {
        require(_tokenXYZ != address(0), "AquasDCA: DCAStorage: tokenXYZ cannot be 0 address");
        StorageID++;
        allStorages[StorageID] = new DCAStorage(StorageID, address(this), Router, USDC_ADDRESS, _tokenXYZ, AQUA_ADDRESS);
    ...
    function _findContractAddress(address _tokenXYZ) private returns (address) {
        require(_tokenXYZ != address(0), "AquasDCA: _findContractAddress: incorrect token");
