...
// A ton of hardcoding usdp addresses in contract. 
// deploy for now and recode for more pairs 
// check for any USDP or RUBY hardcoding 
contract DCAStorage is Ownable {
    ...
    address public AQUA_ADDRESS; // Ruby Token
    ...
    address private USDC_ADDRESS; // Base Token
    ...
    constructor(
        ...
        pathSell[0] = TokenXYZ;
        pathSell[1] = USDC_ADDRESS;

        // buy route
        pathBuy = new address[](2);
        pathBuy[0] = USDC_ADDRESS;
        pathBuy[1] = TokenXYZ;
    }
                ...
                if (OrderList[i].tokenPriceMin < price && OrderList[i].tokenPriceMax > price) {
                    ...
                    if (OrderList[i].buyOrder == true && _buyOrder == true) {
                        ...
                        if (OrderList[i].lastSwapTime < diff && swapAmount >= 1 && minOut >= 1) {
                            ...
                            _swap(USDC_ADDRESS, TokenXYZ, swapAmount, minOut, OrderList[i].trader);
                        ...
                        if (OrderList[i].lastSwapTime < diff && swapAmount >= 1 && minOut >= 1) {
                            _swap(TokenXYZ, USDC_ADDRESS, swapAmount, minOut, OrderList[i].trader);
    ...
    // sends USDC_ADDRESS token to SC
    ...
    // sends USDC_ADDRESS token to User
