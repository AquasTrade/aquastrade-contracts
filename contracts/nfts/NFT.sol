...
contract NFT is ERC721Upgradeable, OwnableUpgradeable, INFT {
    ...
    function initialize(
        ...
        require(_owner != address(0), "NFT: Invalid owner address");
        require(bytes(_description).length != 0, "NFT: Invalid description");
        require(bytes(_visualAppearance).length != 0, "NFT: Invalid visual appearance");
    ...
    function mint(address to) external virtual override onlyMinter {
        require(to != address(0), "NFT: Invalid Receiver");
    ...
    function setMinter(address minter, bool allowance) external virtual override onlyOwner {
        require(minter != address(0), "NFT: Invalid minter address");
