...
// MasterChef copied from SushiSwap
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of RUBY tokens
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlock; // Last block number that RUBY tokens distribution occurs.
        uint256 accRubyPerShare; // Accumulated RUBY tokens per share, times 1e12. See below.
    }
    // The RUBY token!
    RubyToken public ruby;
    // Dev address.
    address public devaddr;
    // Block number when bonus RUBY period ends.
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 rubyReward =
                multiplier.mul(rubyPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accRubyPerShare = accRubyPerShare.add(
                rubyReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accRubyPerShare).div(1e12).sub(
                    user.rewardDebt
                );
