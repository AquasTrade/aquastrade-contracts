...
const func: DeployFunction = async (hre: HardhatRuntimeEnvironment) => {
  ...
  for (let addr of investors.duration12mo) {
    await deploy(`Presale_${addr.toLowerCase()}`, {
      contract: "Vester",
      from: deployer,
      args: [addr.toLowerCase(), TIMESTAMP_NOW, 0, DURATION_12MO, true],
      log: true,
  ...
  for (let addr of investors.duration18mo) {
    await deploy(`Presale_${addr.toLowerCase()}`, {
      contract: "Vester",
      from: deployer,
      args: [addr.toLowerCase(), TIMESTAMP_NOW, 0, DURATION_18MO, true],
      log: true,
      deterministicDeployment: false,
    });
  }
  ...
  for (let addr of investors.duration48mo) {
    await deploy(`Presale_${addr.toLowerCase()}`, {
      contract: "Vester",
      from: deployer,
      args: [addr.toLowerCase(), TIMESTAMP_NOW, 0, DURATION_48MO, true],
      log: true,
      deterministicDeployment: false,
    });
  }
